...
class TestAPI(unittest.TestCase):
    ...
    def setUp(self):
        self.deps.state.state_file_path = tempfile.mktemp(
            suffix=".json"
        )
    ...
    def test_full_oauth_and_fetch_flow(self):
        ...
        session_cookie = resp.cookies.get(
            "netra_session"
        )
    ...
    def test_upload_video_and_queue_analysis(self):
        ...
        # Analysis now starts asynchronously instead of returning
        # the old queued_placeholder state.
        #
        # Since this unit test intentionally uploads fake MP4 bytes,
        # the worker may move from "starting" to "processing" or
        # "failed" before the response/test assertion executes.
        self.assertIn(
    ...
    def test_live_analysis_queue(self):
        ...
        # Live sources are registered as queued and are then handled
        # by the live-monitor pipeline.
        self.assertEqual(
            body["analysis"]["status"],
            "queued",
        )
