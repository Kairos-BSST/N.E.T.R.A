...
def list_recent_alerts(limit: int = 50, job_id: Optional[str] = None) -> List[Dict[str, Any]]:
    with _lock:
    ...
    if job_id:
        ...
        for item in items:
            ...
            if stream.get("job_id") == job_id or event.get("job_id") == job_id:
                filtered.append(item)
        items = filtered
    return items[: max(1, min(limit, _MAX_RECENT))]
...
def _extract_clip(
    ...
    try:
        ...
        database.add_evidence(job_id, user_id, "clip", out_path, filename, database.hash_file(out_path))
        return f"/evidence/clips/{job_id}/{filename}"
    except Exception:
...
def _deliver_alert(
    *,
    ...
    # Fallback: env WEBHOOK_URLS via legacy helper if no JSON routes
    if not destinations:
